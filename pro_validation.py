...
class model(object):
    ...
    def Pro_Validation(self):
        ...
        with tf.Session() as sess:
            ...
            saver.restore(sess,self.model_path+'DCN_up')
            
            for load,data,normal in data_loader.val_fusion_loader(batch_len=self.batch,val_len=self.val_len,skip_len=0,is_vali=True):
                ...
                for i in range(self.batch):
                    up_value.append(result[i][0]*load_std[0]+load_mean[0])
                    
                for i in range(self.batch):
                   load_value.append(load[i][0]*load_std[0]+load_mean[0])

        with tf.Session() as sess:
            saver.restore(sess,self.model_path+'DCN_down')
            ...
            for load,data,normal in data_loader.val_fusion_loader(batch_len=self.batch,val_len=self.val_len,skip_len=0,is_vali=True):
                ...
                for i in range(self.batch):
                    down_value.append(result[i][0]*load_std[0]+load_mean[0])

        up_value=np.array(up_value)
        down_value=np.array(down_value)
        load_value=np.array(load_value)

        up_compare=(load_value>up_value)
        down_compare=(load_value<down_value)

        WS_score=np.zeros((up_value.shape[0],),dtype=np.float32)
        WS_score=up_compare*(load_value-up_value)/load_value+down_compare*(down_value-load_value)/load_value+(up_value-down_value)*2/(up_value+down_value)
        print 'Relative winkler score:' ,np.mean(WS_score)
