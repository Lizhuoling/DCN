...
class model(object):
    ...
    def validation(self):
        ...
        with tf.Session() as sess:
            ...
            saver.restore(sess,self.model_path+'DCN')
            load_list=[]
            label_list=[]
            ...
            for load,data,normal in data_loader.val_fusion_loader(batch_len=self.batch,val_len=self.val_len):
                ...
                load_r=(load-load_mean)/load_std

                feed_dict={x:data,y_:load_r,keep_prob:1}
                y_ter=sess.run(y,feed_dict=feed_dict)
                y_ter=y_ter*load_std+load_mean
                y_ter=y_ter.reshape(y_ter.shape[0],)
                load=load.reshape(load.shape[0],)
                
                load_list=load_list+y_ter.tolist()
                label_list=label_list+load.tolist()
        
            load_list=self.moving_average(load_list)
